...
const API_KEY_STORAGE = "milk-anthropic-key";
...
function SetupScreen({ onSave }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", minHeight:"100vh", background:"#F7F3EC", fontFamily:"'Inter',system-ui,sans-serif" }}>
      <div style={{ background:"#1C2B3A", padding:"60px 24px 32px" }}>
        <div style={{ width:56, height:56, background:"#C9A84C", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, marginBottom:16 }}>🥛</div>
        <div style={{ fontFamily:"Georgia,serif", fontSize:24, color:"#fff", fontWeight:700 }}>Preços do Leite <span style={{ color:"#C9A84C" }}>UE</span></div>
        <div style={{ fontSize:13, color:"#8EA6BB", marginTop:6 }}>Configuração inicial — apenas uma vez</div>
      </div>

      <div style={{ padding:"32px 24px", flex:1 }}>
        <div style={{ background:"#FDFAF5", border:"1px solid #D6CEBD", borderRadius:12, padding:"20px" }}>
          <div style={{ fontWeight:700, fontSize:16, marginBottom:8 }}>🔑 Chave API Anthropic</div>
          <p style={{ fontSize:13, color:"#3D5166", lineHeight:1.7, marginBottom:20 }}>
            Para o botão <strong>Actualizar</strong> funcionar e ir buscar os preços mais recentes, precisas de uma chave API gratuita da Anthropic.
          </p>

          <div style={{ fontWeight:600, fontSize:13, marginBottom:16, color:"#1C2B3A" }}>Como obter (2 minutos):</div>
          {[
            ["1", "Abre o Safari numa aba nova"],
            ["2", "Vai a console.anthropic.com"],
            ["3", "Cria conta grátis (ou faz login)"],
            ["4", 'Clica em "API Keys" no menu'],
            ["5", 'Clica "Create Key" → copia a chave'],
            ["6", "Cola aqui em baixo e guarda"],
          ].map(([n, t]) => (
            <div key={n} style={{ display:"flex", gap:12, marginBottom:10, alignItems:"flex-start" }}>
              <div style={{ width:24, height:24, background:"#C9A84C", borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:700, color:"#1C2B3A", flexShrink:0 }}>{n}</div>
              <div style={{ fontSize:13, color:"#3D5166", paddingTop:3 }}>{t}</div>
            </div>
          ))}

          <div style={{ marginTop:20 }}>
            <div style={{ fontSize:12, color:"#8EA6BB", marginBottom:6 }}>Cola a tua chave aqui:</div>
            <div style={{ display:"flex", gap:8 }}>
              <input
                ...
                style={{ flex:1, padding:"12px", border:"1.5px solid #D6CEBD", borderRadius:8, fontSize:13, background:"#fff", outline:"none", fontFamily:"monospace" }}
              />
              <button onClick={() => setShowKey(p => !p)}
                style={{ padding:"12px", border:"1px solid #D6CEBD", borderRadius:8, background:"#F7F3EC", cursor:"pointer", fontSize:16 }}>
                {showKey ? "🙈" : "👁️"}
              </button>
            </div>
          </div>
          ...
          <button
            onClick={() => { if (key.startsWith("sk-ant")) { localStorage.setItem(API_KEY_STORAGE, key); onSave(key); } else alert("Chave inválida — deve começar com sk-ant"); }}
            style={{ width:"100%", marginTop:16, padding:"14px", background: key.length > 10 ? "#C9A84C" : "#D6CEBD", border:"none", borderRadius:10, fontSize:15, fontWeight:700, color: key.length > 10 ? "#1C2B3A" : "#8EA6BB", cursor: key.length > 10 ? "pointer" : "default" }}>
            Guardar e entrar na app ✓
          </button>
        </div>

        <button onClick={() => onSave(null)}
          style={{ width:"100%", marginTop:12, padding:"12px", background:"transparent", border:"1px solid #D6CEBD", borderRadius:10, fontSize:13, color:"#8EA6BB", cursor:"pointer" }}>
          Entrar sem chave (dados fixos, sem actualização)
        </button>

        <div style={{ marginTop:16, padding:"12px 16px", background:"#eef6f0", border:"1px solid #a8d5b5", borderRadius:8 }}>
          <div style={{ fontSize:11, color:"#3a6e4a", lineHeight:1.65 }}>
            🔒 <strong>A chave fica guardada apenas no teu iPhone</strong>, nunca é enviada para nenhum servidor nosso. É usada directamente para ir buscar os preços à Anthropic.
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const savedData = store.get(STORAGE_KEY);
    if (savedKey) { setApiKey(savedKey); setSetupDone(true); }
    else setSetupDone(false);
    setData(savedData || BASELINE);
  }, []);
...
Valores confirmados Abril 2026: PT=44.43, EU=42.79, CY=66.82, LT=37.46, RO=40.05.`;
    ...
    try {
      ...
      if (!r.ok) throw new Error(`Erro ${r.status} — verifica a chave API`);
      const json = await r.json();
      const text = json.content?.find(b => b.type === "text")?.text || "";
      const parsed = JSON.parse(text.replace(/```json|```/g, "").trim());
      const merged = { ...BASELINE, ...parsed, trend: data?.trend || BASELINE.trend, fetchedAt: new Date().toISOString() };
      setData(merged);
  ...
  if (!setupDone) return <SetupScreen onSave={handleSetup} />;
  if (!data) return (
  ...
  const sorted = [...data.countries]
    ...
    .sort((a, b) => sortBy === "cur" ? b.cur - a.cur : sortBy === "prev" ? b.prev - a.prev : parseFloat(pct(a.cur, a.prev)) - parseFloat(pct(b.cur, b.prev)));

  const chartData = (data.trend?.months || BASELINE.trend.months).map((m, i) => {
    const obj = { month: m };
    Object.keys(SERIES_META).forEach(code => { const s = (data.trend || BASELINE.trend).series[code]; if (s?.[i] != null) obj[code] = s[i]; });
    return obj;
  });
    ...
    <div style={{ display:"flex", flexDirection:"column", height:"100vh", background:"#F7F3EC", fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      {showKeyModal && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.6)", zIndex:100, display:"flex", alignItems:"flex-end" }}>
          <div style={{ background:"#FDFAF5", borderRadius:"16px 16px 0 0", padding:"24px", width:"100%" }}>
            <div style={{ fontWeight:700, fontSize:16, marginBottom:8 }}>🔑 Adicionar Chave API</div>
            <p style={{ fontSize:13, color:"#3D5166", marginBottom:16, lineHeight:1.6 }}>Vai a <strong>console.anthropic.com</strong> → API Keys → cria uma chave e cola aqui.</p>
            <input type="password" placeholder="sk-ant-api03-..."
              onChange={e => { if (e.target.value.startsWith("sk-ant")) { localStorage.setItem(API_KEY_STORAGE, e.target.value); setApiKey(e.target.value); setShowKeyModal(false); addLog("🔑 Chave guardada!"); } }}
              style={{ width:"100%", padding:"12px", border:"1.5px solid #D6CEBD", borderRadius:8, fontSize:13, marginBottom:12, fontFamily:"monospace" }} />
            <button onClick={() => setShowKeyModal(false)} style={{ width:"100%", padding:"12px", background:"#D6CEBD", border:"none", borderRadius:8, fontSize:14, cursor:"pointer" }}>Cancelar</button>
          </div>
        </div>
      )}
      ...
      <div style={{ background:"#1C2B3A", paddingTop:`calc(${safeTop} + 12px)`, paddingBottom:12, paddingLeft:16, paddingRight:16, flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              <div style={{ fontFamily:"Georgia,serif", fontSize:16, color:"#fff", fontWeight:700 }}>Preços do Leite <span style={{ color:"#C9A84C" }}>UE</span></div>
          ...
          <button onClick={runUpdate} disabled={loading}
            ...
            {loading ? <><div style={{ width:12, height:12, border:"2px solid rgba(28,43,58,0.3)", borderTop:"2px solid #1C2B3A", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />…</> : "⟳ Actualizar"}
          </button>
        </div>
        {log.length > 0 && (
          <div style={{ marginTop:8, fontSize:11, color:"#8EA6BB", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"14px 16px 8px" }}>
          ...
          <div style={{ animation:"fadeIn 0.25s ease" }}>
            <div style={{ display:"flex", gap:8, marginBottom:12, alignItems:"center" }}>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍 País…"
                style={{ flex:1, padding:"8px 12px", border:"1px solid #D6CEBD", borderRadius:8, fontSize:13, background:"#FDFAF5", outline:"none" }} />
            ...
            <div style={{ fontSize:10, color:"#8EA6BB", marginTop:10, lineHeight:1.6 }}>Fonte: {data.source}</div>
          </div>
        )}
        ...
        {tab === "chart" && (
          <div style={{ animation:"fadeIn 0.25s ease" }}>
            ...
            <div style={{ fontSize:12, color:"#8EA6BB", marginBottom:14 }}>Verde = acima da média UE ({fmt(data.euAvg)} €)</div>
        ...
        {tab === "variacao" && (
          <div style={{ animation:"fadeIn 0.25s ease" }}>
            ...
            <div style={{ fontSize:12, color:"#8EA6BB", marginBottom:14 }}>Abr 2026 vs Abr 2025 · da maior queda</div>
            <div style={{ background:"#FDFAF5", border:"1px solid #D6CEBD", borderRadius:12, padding:"14px 16px" }}>
      ...
      <div style={{ background:"#FDFAF5", borderTop:"1px solid #D6CEBD", display:"flex", paddingBottom:`calc(${safeBottom})`, flexShrink:0 }}>
        ...
        <button onClick={() => { localStorage.removeItem(API_KEY_STORAGE); setApiKey(null); setSetupDone(false); }}
          style={{ flex:1, padding:"10px 4px 8px", border:"none", background:"none", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
          <span style={{ fontSize:20 }}>🔑</span>
          <span style={{ fontSize:10, fontWeight:600, color:"#8EA6BB" }}>Chave</span>
        </button>
      </div>
    </div>
  );
